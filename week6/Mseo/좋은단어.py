...
보고서 작성이 너무 지루했던 평석이는 노트북에 엎드려서 꾸벅꾸벅 졸다가 
제출 마감 1시간 전에 깨고 말았다. 안타깝게도 자는 동안 키보드가 잘못 눌려서 보고서의 모든 글자가 A와 B로 바뀌어 버렸다! 
...
문제 해석
: 말만 조금 다르지 그냥 괄호 문제랑 똑같다
문제 해석을 왜 이따구로,,,무슨 말인지 한참을 고민했다ㅋㅋㅋ

내가 좋은 단어를 왜 알아햐자????? < - 내용 수정
"""
import sys
...
for _ in range(int(I())):
    ...
    stack_=[0]
    for i in s:
        if(i=='A' and stack_[-1]=='A'):
            stack_.pop()
        elif(i=='B' and stack_[-1]=='B'):
            stack_.pop()
        else:
            stack_.append(i)
    if(len(stack_)==1):
